...
namespace EFCoreTesting.Models
{
    public interface IWork2510ModelRepo2 : IDisposable
    {
        ...
        Task<bool> UpdateUser1Async(long id, User user);
    }
    ...
    public class Work2510ModelRepo2 : IWork2510ModelRepo2
    {
        ...
        public async Task<bool> UpdateUser1Async(long id, User user)
        {
            var list = await context2510.Users.AsNoTracking().ToListAsync();
            var userec = list.Find(c => c.Id == id);
            if (userec == null)
            {
                return false;
            }
